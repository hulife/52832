...
static int minilzo_compress_file(int fd_in, int fd_out)
{
    ...
    lzo_uint8_t *cmprs_buffer = NULL, *buffer = NULL;
    lzo_uint8_t buffer_hdr[BLOCK_HEADER_SIZE] = { 0 };
    ...
    cmprs_buffer = (lzo_uint8_t *) malloc(COMPRESS_BUFFER_SIZE + BUFFER_PADDING);
    buffer = (lzo_uint8_t *) malloc(COMPRESS_BUFFER_SIZE);
    ...
    printf("\n");
    printf("[minilzo]compressed %d bytes into %d bytes , compression ratio is %d%!\n", file_size, totle_cmprs_size,
...
static int minilzo_decompress_file(int fd_in, int fd_out)
{
    ...
    lzo_uint8_t *dcmprs_buffer = NULL, *buffer = NULL;
    lzo_uint8_t buffer_hdr[BLOCK_HEADER_SIZE] = { 0 };
    ...
    {
        printf("[minilzo] decomprssion file size : %d error!\n", file_size);
        ret = -1;
        goto _dcmprs_exit;
    }

    dcmprs_buffer = (lzo_uint8_t *) malloc(DCOMPRESS_BUFFER_SIZE);
    buffer = (lzo_uint8_t *) malloc(DCOMPRESS_BUFFER_SIZE + BUFFER_PADDING);
...
int minilzo_test(int argc, char ** argv)
{
    ...
    */
    ...
    if (fd_out < 0)
    {
        printf("[minilzo] open the output file : %s error!\n", argv[3]);
        ret = -1;
        goto _exit;
    }
